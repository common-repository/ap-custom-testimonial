...
<div class="testim-template" id="<?php echo $testimonial_template; ?>">
    <div class="<?php echo $testimonial_layouts; ?>" data-slide-controls="<?php echo $disp_slider_cntrl; ?>" data-slide-pager="<?php echo $disp_slider_pager; ?>" data-slide-speed="<?php echo $slider_speed; ?>" data-slide-auto="<?php echo $auto_slide; ?>" data-pause-duration="<?php echo $pause_duration; ?>" data-adaptive-height="<?php echo $adaptive_height; ?>" data-slide-mode="<?php echo $slider_mode; ?>" id="apct-layouts"> 
        ...
        foreach ($testimonial_details as $row) {
            ...
            $testimonial_image = isset($testim_description['author_image_path']) && !empty($testim_description['author_image_path']) ? esc_url($testim_description['author_image_path']) : '';
            $author_description = html_entity_decode(stripslashes_deep($testim_description['author_description']));
    ...
    <?php if (isset($testimonial_layout) && $testimonial_layout == 'list-layout') {
        echo $testimonial_layouts_child = 'apct-grid-boxes';
    } ?>
                        <?php if (isset($display_image) && $display_image == 'hide') {
                            echo 'apct-no-img-class';
                        } ?>">
                <div class="apct-upper-scetion-wrapper">
                    <div class="apct-testimonial-content">
    <?php
    if ((isset($content_length)) && $content_length == 'full') {
        echo $author_description;
    } else if ((isset($content_length)) && $content_length == 'excerpt') {
        ?>
                                <?php
                                if (strlen($author_description) >= 175) {
                                    echo substr($author_description, 0, 175) . '...';
                                } else {
                                    echo $author_description;
                                }
                            }
...
<?php } ?>
    ...
    <div id="apct-bxslider-controller-3" class="apct-bxslider-controller" <?php if (isset($testimonial_layout) && $testimonial_layout !== 'slider-layout') {
